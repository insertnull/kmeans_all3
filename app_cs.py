...
best_score = -np.inf
for k in range(2, 6):
    labels, _ = custom_kmeans_cosine_similarity(user_genre_ratings_filtered_scaled, n_clusters=k)
    score = calinski_harabasz_score(user_genre_ratings_filtered_scaled, labels)
    if score > best_score:
        best_k = k
        best_score = score

clusters_enhanced, _ = custom_kmeans_cosine_similarity(user_genre_ratings_filtered_scaled, n_clusters=best_k)
